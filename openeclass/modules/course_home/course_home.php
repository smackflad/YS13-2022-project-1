...
$sql = 'SELECT `description`,`course_keywords`, `course_addon`,`faculte`,`lastEdit`,`type`, `visible`, `titulaires`, `fake_code` FROM `cours` WHERE `code` = ?';
$res = run_Query($sql, array("s", $currentCourse), $mysqlMainDb);
...
$result = run_Query("SELECT MAX(`order`) FROM course_units WHERE course_id = ?", array("s", $cours_id));
list($maxorder) = $result->fetch_row();

// other actions in course unit
if ($is_adminOfCourse) {
    if (isset($_REQUEST['edit_submit']) && (isset($_POST['_token']) || ($_POST['_token'] == $_SESSION['_token']))) {
        ...
        if (isset($_REQUEST['unit_id'])) { // update course unit
            ...
            $result = run_Query("UPDATE course_units SET
                                                   title = ?,
                                                   comments = ?
                                            WHERE id = ? AND course_id = ?", array("ssss", $title, $descr, $unit_id, $cours_id));
            $main_content .= "\n      <p class='success_small'>$langCourseUnitModified</p>";
        } else { // add new course unit
            $order = $maxorder + 1;
            run_Query("INSERT INTO course_units SET
                                         title = ?, comments =  ?,
                                         `order` = ?, course_id = ?", array("ssss", $title, $descr, $order, $cours_id));
            $main_content .= "\n        <p class='success_small'>$langCourseUnitAdded</p>";
        }
    } elseif (isset($_REQUEST['del'])) { // delete course unit
        $id = intval($_REQUEST['del']);
        run_Query("DELETE FROM course_units WHERE id = ?", array("s", $id));
        run_Query("DELETE FROM unit_resources WHERE unit_id = ?", array("s", $id));
        ...
        $sql = run_Query("SELECT `visibility` FROM course_units WHERE id=?", array("s", $id));
        list($vis) = $sql->fetch_row();
        $newvis = ($vis == 'v')? 'i': 'v';
        run_Query("UPDATE course_units SET visibility = ? WHERE id = ? AND course_id = ?", array("sss", $newvis, $id, $cours_id));
...
$cunits_content .= "</td>\n      </tr>\n      </thead>\n      </table>\n";
if ($is_adminOfCourse) {
    list($last_id) = run_Query("SELECT id FROM course_units
                                                   WHERE course_id = ?
                                                   ORDER BY `order` DESC LIMIT 1", array("s", $cours_id))->fetch_row();
...
$sql = run_Query($query, array("s", $cours_id));
$first = true;
$count_index = 1;
...
if ($is_adminOfCourse) {
    $sql = "SELECT COUNT(user_id) AS numUsers
			FROM cours_user
			WHERE cours_id = ?";
    $res = run_Query($sql, array("s", $cours_id), $mysqlMainDb);
    while($result = $res->fetch_row()) {
        $numUsers = $result[0];
    }
