...
/*public*/ class PMA_List_Database extends PMA_List
{
    ...
    protected function _checkAgainstPrivTables()
    {
        ...
        $local_query = "
            ...
            WHERE `Select_priv` = 'Y'
            AND `User`
            IN ('" . PMA_sqlmysql_real_escape_string($GLOBALS['cfg']['Server']['user']) . "', '')";
        ...
        $local_query = 'SELECT DISTINCT Db FROM mysql.tables_priv WHERE Table_priv LIKE \'%Select%\' AND User = \'' . PMA_sqlmysql_real_escape_string($GLOBALS['cfg']['Server']['user']) . '\'';
